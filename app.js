...
// public 디렉토리 안에 있는 파일들을 재공 -> 직접 읽어서 전송하지 않아도 됨.
...
app.use(cookieParser(process.env.COOKIE_SECRET));
    // COOKIE_SERRET -> 제공한 비밀키를 통해 해당 쿠키가 내 서버가 만든 쿠키임을 검증
    // 쿠키 -> 서버와 클라이언트 간의 상태를 유지하고 정보를 교환하는데 사용하는 작은 데이터 조각, 웨브라우저에 저장됨.
...
app.use(session({
    ...
    // resave -> 요청이 올 떄 세션에 수정사항이 생기지 않더라도 세션을 다시 저장할지 설정
    // saveUninitialized -> 세션에 저장할 내역이 없더라도 처음부터 세션을 생성할지 설정
    // secret -> 안전하기 쿠키를 전송하려면 쿠키에 서명을 추가해야함. 쿠키에 서명할 때 secret 값이 팔요
    // cookie -> 세션 쿠키에 대한 설정
...
app.get('/', (req, res, next) => {
    ...
    // (req, res) => {} -> 다음으로 실행될 콜백 함수를 정의, 이 함수는 req, res를 인자 받음.
    // throw net Error -> 에러를 발생시키고, 이를 등록된 에러 처리 미들웨어로 전달

// 에러처리 미들웨어 -> 매개 변수가 err, req, res, next로 4개 -> 반드시 매개변수가 4개여야 함.
// 기본적으로 익스프레스가 에러를 처리하긴 하지만, 직접 에러 처리 미들웨어를 연결해두는 것을 권장
// 에러처리 미들웨어는 특별한 경우가 아니면 가장 아래에 위치함.
app.use((err, req, res, next) => {
    console.error(err);
    res.status(500).send(err.message);
})
    // err -> 에러에 관한 덩보가 담겨 있음
    // res.status(HPPT 상태코드) -> HPPT 상태코드 지정, 기본 값은 200(성공)
